...
class Dropdown extends Component {
      ...
      return (
        <div className="col-sm-3 mt-4" key={classroom.id}>
          <Link to={`/classrooms/${classroom.id}`}>
            <div className="card">
              <div className="card-img-top" style={{ height: "120px", backgroundColor: "#cfccff", overflow: "hidden", position: "relative" }}>
                <img src={notebook} alt="notebook" width="100%" style={{ position: "relative", top: "-30px" }} />
              </div>
  ...
  render() {
    return (
      <>
        <Link className="nav-link px-3" onClick={this.showMenu} to="/">{this.props.title}</Link>
        {
          this.state.showMenu
            ? (
              <div
              ...
              >
                <div className="row">
                  <div className="col-sm-12">
                    <h1 style={{ color: "#000", fontWeight: "300", fontSize: "30px", textTransform: "capitalize" }}>Classrooms</h1>
                  </div>
                </div>
                <div className="row">
